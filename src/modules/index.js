...
import { getDate, mergeOptions } from './util'

// 创建新实例
const container = document.createElement('div')
document.body.appendChild(container)
const app = createApp(PickerCom)
...
const Picker = {
  show: (settings = {}) => {
    ...
    if (settings.startTime && settings.endTime) {
      if (
        new Date(getDate(settings.startTime)).getTime() >=
        new Date(getDate(settings.endTime)).getTime()
      ) {
        years[0] = 1949
    ...
    settings.dateList = [yearsList, monthsList, daysList, hourList, minuteList]
